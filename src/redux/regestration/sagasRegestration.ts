...
export function* doRegistration(): IterableIterator<any>{
    yield takeEvery(RPr.DO_REGISTER, function*(obj:any){
        ...
        let response = yield call(request, "POST", mainObj)
        ...
        }else if(response.success === false && response.errorValid === false){
            yield put({type:RPr.USER_EXSIST, error: response.message})
        }else if(response.success === false && response.errorValid === true){
